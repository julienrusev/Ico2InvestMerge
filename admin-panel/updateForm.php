			...
			<div class="text_area">
				...
				<form action="updateToDB.php" method="POST" name="update_form" class="update_form">
					<div class="details">
						...
						<label>Name</label> 
						<input type="text" name="name" value="<?php echo $dataArray[0]['NAME'] ?>" class="readonly_name" readonly><br>
						<label>Start date</label>
						<input type="date" name="start_date" value="<?php echo substr($dataArray[0]['START_DATE'], 0, 10) ?>" class="start_date"><br>
						<label>End date</label>
						<input type="date" name="end_date" value="<?php echo substr($dataArray[0]['END_DATE'], 0, 10) ?>" class="end_date"><br>
						<label>ICO Price</label>
						<input type="number" name="ico_price" value="<?php echo $dataArray[0]['ICO_PRICE'] ?>" class="ico_price"><br>
						<label>ICO Quantity</label>
						<input type="number" name="ico_quantity" value="<?php echo $dataArray[0]['ICO_QTY'] ?>" class="ico_quantity"><br>
						<div class="status_holder">
							<label>ICO Status</label>
							<div class="range">
								<button class="zero" type="button">Inactive</button>
								<input type="range" class="slider" value="<?php echo $dataArray[0]['ICO_STATUS'] ?>" name="ico_status" id="test" min="0" max="1">
								<button class="one" type="button">Active</button>
							</div><br>
						</div> 
						<span class="descr">Description</span><br>
						<textarea name="description" id="" cols="40" rows="5" class="ico_description"><?php echo $dataArray[0]['DESCRIPTION'] ?>
						</textarea><br>
						<span class="descr">Short Description</span><br>
						<textarea name="short_description" id="" cols="40" rows="5" class="ico_short_description"><?php echo $dataArray[0]['SHORT_DESCRIPTION'] ?>
						</textarea><br>
					</div>
					<div class="pos_holder">
						...
						<?php 
							...
							foreach($dataArray as $value){
								if($value['STATUS'] == 1) {
									echo "<div class='textarea_holder_pos'>".
											"<input type='text' name='positives[$value[ID_AUTO]][$title]' value='$value[TITLE]' class='pos_title'>".
											"<textarea name='positives[$value[ID_AUTO]][$text]' id='$value[ID_AUTO]' class='positives' cols='40' rows='5'>".
												$value["INFO"]. 
											"</textarea>".
											"<button type='button' id='$value[ID_AUTO]' class='remove'>Delete</button>".
										 "</div>". "<br>";
								}
							}
						?>
								...
								$('#add_positive').on('click', function(){
									$('#add_positive').before(
										"<div class='textarea_holder_pos'>\
											<input type='text' name='newPositive["+index+"][title]' class='pos_title'>\
											<textarea name='newPositive["+index+"][text]' class='positives' cols='40' rows='5'></textarea>\
					...
					<div class="neg_holder">
						...
						<?php 
							...
							foreach($dataArray as $value){
								if($value['STATUS'] == 0){
									echo "<div class='textarea_holder_neg'>".
											"<input type='text' name='negatives[$value[ID_AUTO]][$title]' value='$value[TITLE]' class='neg_title'>".
											"<textarea name='negatives[$value[ID_AUTO]][$text]' id='$value[ID_AUTO]' class='negatives' cols='40' rows='5'>".
												$value["INFO"].
											"</textarea>".
											"<button type='button' id='$value[ID_AUTO]' class='remove'>Delete</button>".
										 "</div>". "<br>";
								...
								$('#add_negative').on('click', function(){
									$('#add_negative').before(
										"<div class='textarea_holder_neg'>\
											<input type='text' name='newNegative["+index+"][title]' class='neg_title'>\
											<textarea name='newNegative["+index+"][text]' class='negatives' cols='40' rows='5'></textarea>\
											<button type='button' class='remove_text_field'>Delete</button>\
										</div><br>"
									);
									index++;
